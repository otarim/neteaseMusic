...
app.config(['$routeProvider','$httpProvider','$sceDelegateProvider','songListControllerProvider','RestangularProvider',function($routeProvider,$httpProvider,$sceDelegateProvider,songListControllerProvider,RestangularProvider){
	...
	$routeProvider.when('/',{
		templateUrl: 'http://7vilgy.com1.z0.glb.clouddn.com/view/index.tpl',
		controller: ['$scope','$http','type','Restangular','song','changeRoute','store','global','musicboxData',function($scope,$http,type,Restangular,song,changeRoute,store,global,musicboxData){
			...
			$scope.search = function(){
				if($scope.searchItem){
					...
					// encode for json
					// $http.post('http://127.0.0.1:9527/search',{s: encodeURIComponent($scope.searchItem),type: $scope.selectedType}).then(function(resp){
			...
			$scope.play = function(id,index){
				$http.get('http://127.0.0.1:9527/song?id='+id).success(function(d){
		...
		templateUrl: 'http://7vilgy.com1.z0.glb.clouddn.com/view/artist.tpl',
		controller: ['songListController','$scope',function(songListController,$scope){
			songListController('artist',$scope)
		}]
	}).when('/album/:id',{
		templateUrl: 'http://7vilgy.com1.z0.glb.clouddn.com/view/album.tpl',
		controller: ['songListController','$scope',function(songListController,$scope){
	...
	return {
		...
		// template: '',
		templateUrl: 'http://7vilgy.com1.z0.glb.clouddn.com/view/musicbox.tpl',
		replace: true,
		// transclude: true,
