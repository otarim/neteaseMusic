...
service.config(['RestangularProvider',function(RestangularProvider){
	RestangularProvider.setBaseUrl('http://192.168.18.105:9527/')
	...
	this.$get = function($http,$routeParams,song,changeRoute,musicboxData){
		...
		return function(type,$scope){
			var url = 'http://192.168.18.105:9527/' +type+'?'+type+'='
...
var app = angular.module('app', ['ngRoute','ngResource','ngTouch','service','restangular'])
...
app.config(['$routeProvider','$httpProvider','songListControllerProvider','RestangularProvider',function($routeProvider,$httpProvider,songListControllerProvider,RestangularProvider){
	...
	RestangularProvider.setBaseUrl('http://192.168.18.105:9527/')
	// songListControllerProvider.url = 'http://192.168.18.105:9527/'
	$routeProvider.when('/',{
		...
		controller: ['$scope','$http','type','Restangular','song','changeRoute','store','global','musicboxData',function($scope,$http,type,Restangular,song,changeRoute,store,global,musicboxData){
			...
			// var restful = $resource('http://192.168.18.105:9527/:action',{
			// 	action: '@action',
			// },{
			// 	// 配置$resource
			// 	init: {
			...
			$scope.search = function(){
				if($scope.searchItem){
					...
					// encode for json
					// $http.post('http://192.168.18.105:9527/search',{s: encodeURIComponent($scope.searchItem),type: $scope.selectedType}).then(function(resp){
					// 	$scope.result = resp.data
					// },function(err){
					// 	console.log(err)
					// })
					// restful.save({},function(data){
						// data.params = ; data.$save($delete...)
					// })
					// restful.save({
					// 	action: 'search'
					// },{
					// 	s: encodeURIComponent($scope.searchItem),type: $scope.selectedType
					// },function(resp){
					// 	$scope.result = resp
					// },function(err){
					// 	console.log(err)
			...
			$scope.play = function(id,index){
				$http.get('http://192.168.18.105:9527/song?id='+id).success(function(d){
			...
			if(store.has('netease.searchItem')){
				...
				// restful.save({},{
				// 	s: encodeURIComponent(store.a),type: store.b
				// },function(resp){
				// 	$scope.result = resp
				// },function(err){
				// 	console.log(err)
