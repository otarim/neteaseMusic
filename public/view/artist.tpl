<div class="artist">
	...
	<ul class="content-songs">
		...
		<li ng-repeat="(key, value) in data.hotSongs | orderBy: order: rev" ng-click="selectSong($index)" ng-class="{active: index === $index,odd: $odd}">
			<span class="song-index">{{$index+1}}<a href="" class="fa fa-play-circle-o" ng-click="play(value,$index)"></a></span><span class="song-title">{{value.name}}</span><span class="song-album"><a href="" ng-click="changeRoute('album',{id: value.album.id})">{{value.album.name}}</a></span><span class="song-time">{{value.duration|formatTime}}</span><span class="song-score"><i><b ng-style="{width: value.score+'%'}"></b></i></span>
		</li>
	</ul>
	<h4>热门专辑:</h4>
	<div class="albums">
